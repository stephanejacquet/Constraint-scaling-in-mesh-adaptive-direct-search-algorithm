...
if ( p==2 )
    X=load('outputs/dp5.txt');
end
if ( p==3 )
   X=load('outputs/dp7.txt');    
end 
...
h(1)=plot ( L1(1:MMM,1) , L1(1:MMM,2)/100 , 's', 'color',cOrange,'LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor',cOrange,'MarkerSize',7 ); 
...
if ( NBcol>=3 )
    ...
    h(2)=plot ( L2(1:MMM,1) , L2(1:MMM,3)/100 , 'o','color','b', 'LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','b','MarkerSize',7 ); 
...
if ( p==1 )
    title ( {'précision \tau=1e-3'} );
end
if ( p==2 )
    title ( {'précision \tau=1e-5'} );
end
if ( p==3 )
    title ( {'précision \tau=1e-7'});
end
