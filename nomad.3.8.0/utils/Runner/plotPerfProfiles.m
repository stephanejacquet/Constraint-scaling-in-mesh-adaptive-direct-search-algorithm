...
h(1)=plot ( L1(:,1) , L1(:,2)/100 , 's', 'color',cOrange,'LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor',cOrange,'MarkerSize',7 ); 
plot( X(1:MM,1) , X(1:MM,2)/100 , 'r--' ); 
...
if ( NBcol>=3 )
    ...
    h(2)=plot ( L2(:,1) , L2(:,3)/100 , 'o', 'color','b','LineWidth',1,'MarkerEdgeColor','k','MarkerFaceColor','b','MarkerSize',7 ); 
    plot( X(1:MM,1) , X(1:MM,3)/100 , 'b--' );
    if ( NBcol==3 )
...
title ( {'précision \tau=1e-7'},'fontsize',17.6 );
    
axis ( [1 maxAxeX  0  1] );

xlabel ( 'ratio de performance','fontsize',17.6 );
ylabel ( 'ratio de problèmes résolus','fontsize',17.6 );
