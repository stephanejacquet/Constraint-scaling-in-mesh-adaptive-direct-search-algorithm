...
{//cout<< "coucou cache \n";
//const vector<NOMAD::Double> maximums_outputs=get_max_of_each_output();
...
#endif
            cout<<"wesh \n";
            vector<NOMAD::Double> my_max=_cache->get_max_of_each_output();
            // set_f, set_h, and set_EB_ok:
            _ev->compute_f ( *modifiable_cache_x );
            _ev->compute_h ( *modifiable_cache_x );
            //_ev->compute_h ( *modifiable_cache_x, my_max );
        }
    }
...
//cin >>s;   
 ...
 if (cur->is_feasible(0)){un_voisin.origin=0;
 
}

 else {un_voisin.origin=1;
