...
void NOMAD::Parameters::read ( const NOMAD::Parameter_Entries & entries )
{
    ...
    // --------------
    {
        ...
        {   std::cout<< "test"<< std::endl;
            pe = entries.find ( "DIVERSIFICATION" );
            if ( pe ) {
                
                if ( !pe->is_unique() )
                    throw Invalid_Parameter ( "Parameters.cpp" , __LINE__ ,
                                             "invalid parameter: DIVERSIFICATION not unique" );
                i = NOMAD::string_to_bool ( *(pe->get_values().begin() ) );
                if ( pe->get_nb_values() != 1 || !NOMAD::atoi (*(pe->get_values().begin()), i) )
                    throw Invalid_Parameter ( "Parameters.cpp" , __LINE__ ,
                                             "invalid parameter: DIVERSIFICATION" );
                set_DIVERSIFICATION ( i );
...
const NOMAD::mesh_type & NOMAD::Parameters::get_mesh_type ( void ) const
{
 ...
 int NOMAD::Parameters::get_diversification ( void ) const
 {
 
    if ( _to_be_checked )
        throw Bad_Access ( "Parameters.cpp" , __LINE__ ,
                          "Parameters::get_diversification(), Parameters::check() must be invoked" ); 
    return(_sort_by_diversification);
...
// set_DIVERSIFICATION:
void NOMAD::Parameters::set_DIVERSIFICATION ( int sbd )
{
    _to_be_checked  = true;
    _sort_by_diversification = sbd;
}
