...
{   
    ...
    for ( int i = 0 ; i < m ; ++i )
    {
        ponderation=0.0;
            ...
            while (cur)
            {
               if ((*cur).get_bb_outputs()[i].is_defined()) {
               if ((*cur).get_bb_outputs()[i]>ponderation && (*cur).get_bb_outputs()[i]<10000000000) ponderation=(*cur).get_bb_outputs()[i]; }
               cur = _cache.next(); 
            }  
