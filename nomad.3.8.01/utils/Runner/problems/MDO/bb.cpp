...
const double LB    [] = { 0.1 , 0.75 , 0.75 , 0.1 , 0.01 , 30000 , 1.4 , 2.5 , 40 ,  500 };
const double RANGE [] = { 0.3 , 0.50 , 0.50 , 0.9 , 0.08 , 30000 , 0.4 , 6.0 , 30 , 1000 };
...
bool read_values ( const string & file_name , double x [10] );
...
bool read_values ( const string & file_name , double x [10] ) {
  ...
  // read and scale [0;100] to [lb;ub]:
  double v;
  for ( int i = 0 ; i < 10 ; i++ ) {
    in >> v;
    x[i] = LB[i] + RANGE[i] * v / 100.0;
  }

  if (in.fail()) {
    in.close();
    if (display)
      cerr << "\ncould not read file " << file_name << endl;
    return false;
  }
