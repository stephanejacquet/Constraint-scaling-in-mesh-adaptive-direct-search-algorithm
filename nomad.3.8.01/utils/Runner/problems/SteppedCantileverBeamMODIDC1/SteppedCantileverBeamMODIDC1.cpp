#include <string>
#include "SteppedCantileverBeamMODIDC1.hpp"
...
/*                  constructor                 */
/*----------------------------------------------*/
SteppedCantileverBeamMODIDC1::SteppedCantileverBeamMODIDC1 ( void )
: Problem ( "SteppedCantileverBeamMODIDC1" , "SteppedCantileverBeamMODIDC1" , "xe.txt" , 10 , 12 ) {
    ...
    string temp;
    int temp2;
    ifstream fich("problems/SteppedCantileverBeamMODIDC1/DOE.plan");
    for (int k=0; k<7; k++)
    {
        getline(fich,temp);
    }
...
bool SteppedCantileverBeamMODIDC1::eval_x ( Eval_Point          & x          ,
...
/*--------------------------------------*/
void EP_SteppedCantileverBeamMODIDC1::construct_extended_points ( const Eval_Point & x )
{
    vector<int> other_types1;
    vector<int> other_types2;
    vector<int> other_types3;
    vector<int> other_types4;
    vector<int> other_types5;
    vector<int> other_types6;
    
    for (int j=1; j<6; j++)
    {
        if (x[0].value() != j)
        {
            other_types5.push_back(j);
        }
    }
    for ( size_t k = 0 ; k < other_types5.size() ; k++ )
    {
        Point y = x ;
        y[0] = other_types5[k];
    ...
    for (int j=30; j<66; j++)
    {
        if (x[5].value() != j)
        {
            other_types6.push_back(j);
        }
    }
    for ( size_t k = 0 ; k < other_types6.size() ; k++ )
    {
        Point y = x ;
        y[5] = other_types6[k];		
        add_extended_poll_point ( y , *(x.get_signature())  );
