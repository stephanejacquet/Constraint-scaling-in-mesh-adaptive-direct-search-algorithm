...
vector<NOMAD::Double> NOMAD::Cache::get_max_of_each_output(void) {
    const NOMAD::Eval_Point * current = this->begin();
    if (!current) {//cout << "ok1 \n";
        vector<NOMAD::Double> max_outputs(100, 1.0);

        return (max_outputs);
    } else {
        int m = current->get_bb_outputs().size();

        //vector<NOMAD::Double> max_outputs(m,-1.0);    
        vector<NOMAD::Double> max_outputs(m, 0.0);

        while (current) {

            for (int i = 0; i < m; i++) {
                //cout << i <<" ";
                if (current->get_bb_outputs().get_coord(i).is_defined()) {
                    if (current->get_bb_outputs().get_coord(i) > max_outputs[i] && current->get_bb_outputs().get_coord(i) < 10000000000000.0) {
                        max_outputs[i] = current->get_bb_outputs().get_coord(i);
                    }
                }
            }
            current = this->next();
        ...
        for (int i=0; i<m; i++){
            //cout << "max_outputs de i est "<<max_outputs[i]<< endl;
        }
        //cout << "ok2 \n";
        return (max_outputs);
    }
...
vector<NOMAD::Double> NOMAD::Cache::get_mean_of_each_output(void) {
    const NOMAD::Eval_Point * current = this->begin();
    if (!current) {//cout << "ok1 \n";
        vector<NOMAD::Double> mean_outputs(100, 1.0);

        return (mean_outputs);
    } else {
        int m = current->get_bb_outputs().size();
        //vector<NOMAD::Double> max_outputs(m,-1.0);    
        vector<NOMAD::Double> mean_outputs(m, 0.0);
        vector<int> compteur(m, 0);
        ...
        while (current) {
            ...
            for (int i = 0; i < m; i++) {
                //cout << i <<" ";
                if (current->get_bb_outputs().get_coord(i).is_defined()) {
                    if (current->get_bb_outputs().get_coord(i) > 0.0 && current->get_bb_outputs().get_coord(i) < 10000000000000.0) {
                        mean_outputs[i] += current->get_bb_outputs().get_coord(i);
                        compteur[i]++;
...
/*
 vector<NOMAD::Double> NOMAD::Cache::get_median_of_each_output ( bool & need_to_recalculate_h)
{cout <<"coucou 1\n";
const NOMAD::Eval_Point * current = this->begin();
  
 vector<bool> h_has_been_recalculated=this->get_h_has_been_recalculated();
//const vector<bool> h_needs_to_be_recalculated_now=this->get_h_needs_to_be_recalculated_now();
if (!current){//cout << "ok1 \n";
    vector<NOMAD::Double> median_outputs(100,1.0);
     
    return(median_outputs); }
else
{
int m  = current->get_m();  
int n = current->get_n();
int n_sur_2=ceil(n/2);
//vector<int> compteurs(m,1.0); 
//vector<NOMAD::Double> max_outputs(m,-1.0);    
vector<NOMAD::Double> median_outputs(m,0.0);
vector< std::set<double> > set_of_outputs(m);
 ...
 while ( current )
   {      
     ...
     for(int i=0; i<m;i++){
         ...
         if (compteur[i]<n){
         if(current->get_bb_outputs().get_coord(i).is_defined()){
         if(current->get_bb_outputs().get_coord(i).value()>0.000000001 && current->get_bb_outputs().get_coord(i).value()<10000000000000.0) 
         {   cout<<"itttttttttttttt = "<< current->get_bb_outputs().get_coord(i).value()<<"\n";
             set_of_outputs[i].insert(current->get_bb_outputs().get_coord(i).value());
             compteur[i]++;
             
            if (current->get_bb_outputs().get_coord(i).value() > 1000000)
                     cin.get();
              
             if (current->get_bb_outputs().get_coord(i).value() < 0.000000000001)
                     cin.get();
...
cout <<"coucou 3\n";
for (int i=0; i<m;i++){
   cout << "pour i= "<< i <<", compteur_i= "<< compteur[i]<<"\n";
   //cout << "pour i= "<< i <<", compteur_i vaut en fait "<< set_of_outputs[i].size()<<"\n";
   if(compteur[i]>=n){
        
   cout <<"doit etre modifie\n";
   std::set<double>::iterator it = set_of_outputs[i].begin() ;
   int j=1;
   cout<<"it= "<< *it<<"\n";
   while(j<n_sur_2){
       if(it!= set_of_outputs[i].end()  ){
       cout<<"i= "<< i<<"\n";
       cout<<"j= "<< j <<"\n";
       cout<<"it= "<< *it<<"\n";
           j++;
   ...
   median_outputs[i]=(*it);
   cout <<" la mediane de la "<< i << "-eme sortie est: " << (*it)<<"\n";
           if(! h_has_been_recalculated[i])
       {
           h_has_been_recalculated[i]=true;  
           need_to_recalculate_h=true;
       }
   }
   else median_outputs[i]=1.0;
}
if (need_to_recalculate_h){
   cout<<"allez, recalcule \n";
   this->recalculate_h();
   cout<<"bon travail \n";
}
this->set_h_has_been_recalculated(h_has_been_recalculated);
return(median_outputs);
}
}
...
const vector<NOMAD::Double> &  NOMAD::Cache::get_median_of_each_output(bool & need_to_recalculate_h) {//cout <<"coucou 1\n";
    const NOMAD::Eval_Point * current = this->begin();

    vector<bool> h_has_been_recalculated = this->get_h_has_been_recalculated();
    //const vector<bool> h_needs_to_be_recalculated_now=this->get_h_needs_to_be_recalculated_now();
    if (!current) {//cout << "ok1 \n";
        _median_outputs.resize(100, NOMAD::Double(1.0));

        return _median_outputs;
    } else {//cout <<"coucou 22\n"; 
        int m = current->get_m();
        int n = current->get_n();
        int n_sur_2 = ceil(n / 2);
        //vector<int> compteurs(m,1.0); 
        //vector<NOMAD::Double> max_outputs(m,-1.0);    
        _median_outputs.resize(m, NOMAD::Double(0.0));
        vector< vector<NOMAD::Double> > set_of_outputs(m);
        vector<NOMAD::Double> zhu(n, NOMAD::Double(0.0));
        for (int i = 0; i < m; i++) {
            set_of_outputs[i] = zhu;
        }

        vector<int> compteur(m, 0);
        while (current) {  //cout <<"coucou 2\n";   

            for (int i = 0; i < m; i++) {
                //cout << i <<" ";
                if (compteur[i] < n) {
                    if (current->get_bb_outputs().get_coord(i).is_defined()) {
                        if (current->get_bb_outputs().get_coord(i) > 0.0 && current->get_bb_outputs().get_coord(i) < 10000000000000.0) { //cout<<"itttttttttttttt = "<< current->get_bb_outputs().get_coord(i).value()<<"\n";
                            set_of_outputs[i][compteur[i]] = (current->get_bb_outputs().get_coord(i));
                            compteur[i]++;
        ...
        for (int i = 0; i < m; i++) {
            //cout << "pour i= "<< i <<", compteur_i= "<< compteur[i]<<"\n";
            sort(set_of_outputs[i].begin(), set_of_outputs[i].end());
            if (compteur[i] >= n) {

                //cout <<"doit etre modifie\n";
                /*std::set<double>::iterator it = set_of_outputs[i].begin() ;
                
                for(int j=1; j<n_sur_2;j++){
                    cout<<"i= "<< i<<"\n";
                    cout<<"j= "<< j <<"\n";
                    ...
                    if ((*it) < 0.000000000001)
                                  cin.get();
        
                }*/
                //median_outputs[i]=(*it);
                _median_outputs[i] = set_of_outputs[i][n_sur_2 - 1];
                //cout <<" la mediane de la "<< i << "-eme sortie est: " << _median_outputs[i]<<"\n";
                if (!h_has_been_recalculated[i]) {
                    h_has_been_recalculated[i] = true;
                    need_to_recalculate_h = true;
                }
            } else _median_outputs[i] = NOMAD::Double(1.0);
        }

        this->set_h_has_been_recalculated(h_has_been_recalculated);
        //cout <<"Capri, c'est fini.\n";
        /*for(int i=0; i<m;i++){
            cout <<" la mediane de la "<< i << "-eme sortie est: " << _median_outputs[i]<<"\n";
        }*/
        return (_median_outputs);
