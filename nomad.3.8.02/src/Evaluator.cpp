...
void NOMAD::Evaluator::compute_h ( NOMAD::Eval_Point & x, const vector<NOMAD::Double> & maxs ) const
{
    if ( x.get_bb_outputs().size() != _p.get_bb_nb_outputs() )
    {   
        std::ostringstream err;
    ...
    for ( int i = 0 ; i < nbo ; ++i )
    {
        ...
        if (maxs[i]==0.0){bboi=bbo[i]/1.0;}
        else{if (bbo[i].is_defined()){
        //cout <<"pour i=  "<<i<<"max i= "<< maxs[i] << "bbo[i]= "<< bbo[i] << "\n";
        bboi = bbo[i]/maxs[i];
        //cout <<"t'as calcule bboi? \n";
        }
        }
        if ( bboi.is_defined()                                  &&
            (bbot[i] == NOMAD::EB || bbot[i] == NOMAD::PEB_E ) &&
             ...
             bbot[i] == NOMAD::PEB_P     ) )
        {
            ...
            //cout <<"h_min vaut "<< _p.get_h_min()<< endl;
            //cout <<" bboi vaut "  << bboi<< endl;
            if ( bboi > _p.get_h_min() )
            {
                switch ( _p.get_h_norm() )
                {
