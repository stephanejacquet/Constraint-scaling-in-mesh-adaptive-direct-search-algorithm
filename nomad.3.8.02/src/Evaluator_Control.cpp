...
bool NOMAD::Voisin::operator<(const Voisin& c1) const {
...
void NOMAD::Evaluator_Control::recalculate_h(vector<NOMAD::Double> medians) {
    //cout <<"recalcule moi cela \n";
    NOMAD::Eval_Point * current = &NOMAD::Cache::get_modifiable_point(*(_cache->begin()));
    while (current) {
        if ((*current).is_eval_ok()) {
            //cout <<"avant "<< (*current).get_h()<<"\n";
            _ev->compute_h(*current, medians);
            //cout <<"apres "<< (*current).get_h()<<"\n \n \n";
        }
        current = &NOMAD::Cache::get_modifiable_point(*(_cache->next()));
...
/*vector<bool> NOMAD::Evaluator_Control::need_to_change( )
{

   vector<bool> have_to_change;
   
   return(have_to_change)
...
bool NOMAD::Evaluator_Control::cache_check(const NOMAD::Eval_Point *& x,
    ...
    if (cache_x) {
        ...
        if (cache_x->is_eval_ok()) {
            ...
            vector<NOMAD::Double> maxs=_cache->get_max_of_each_output();
            //cout <<"compute_h 0\n";
            _ev->compute_h(*modifiable_cache_x, maxs);
            //cout <<"compute_h 0\n";
            //bool need_to_recalculate = false;
            //cout <<"avant 0\n";
            //const NOMAD::Eval_Point *h_max_point = true_barrier.get_best_infeasible();
            //cout <<"h max vaut avant "<< true_barrier.get_h_max() <<"\n";
            //const vector<NOMAD::Double> & medians = _cache->get_median_of_each_output(need_to_recalculate);
            //cout <<"apres 0\n";
            
                // taking the point used for new_h_max to update h_max
                

                //NOMAD::Barrier & barrier = (_p.get_opt_only_sgte()) ? sgte_barrier : true_barrier;

                //recalculate all h for the cache
                // recalculate_h(medians);
            //cout <<"update h 0\n";
                NOMAD::Eval_Point * current = &NOMAD::Cache::get_modifiable_point(*(_cache->begin()));
                while (current) {
                    //cout <<"allez 0\n";
                    _ev->compute_h(*current, maxs);
                    
                    current = &NOMAD::Cache::get_modifiable_point(*(_cache->next()));
                }
                //cout <<"update h 0\n";
                //NOMAD::Double new_h_max = h_max_point->get_h();
                
                //true_barrier.set_h_max(new_h_max);
                //cout <<"h max vaut apres "<< true_barrier.get_h_max() <<"\n \n \n";
            
            //vector<NOMAD::Double> maxs(10,1.0);
...
std::set<const NOMAD::Eval_Point*> NOMAD::Evaluator_Control::copy_cache(NOMAD::dd_type display_degree,
        bool & modified_list) {
    std::set<const NOMAD::Eval_Point*> my_cache;
    const NOMAD::Eval_Point * current = _cache->begin();


    while (current) {


        my_cache.insert(current);
...
void NOMAD::Evaluator_Control::classification_adaptated_one_binary_constraint_ordering_basic_diversification_normalised(NOMAD::dd_type display_degree,
        bool & modified_list) {
    //std::cout<<"debut";
    std::set<const NOMAD::Eval_Point*> my_cache = copy_cache(display_degree, modified_list);
    //int nb_voisins=6; 
    //int nb_voisins=leave_one_out_cross_validation_epanechnikov (  display_degree , modified_list, my_cache);
    //std::cout << "nb_voisins="<< nb_voisins << "\n ";
    int n = -1;
    int i;
    NOMAD::eval_type ev_type = NOMAD::TRUTH;
    NOMAD::Point min, max, center, interpolation_radius;
    const NOMAD::Signature * signature = NULL;
    std::set <NOMAD::Voisin> mes_voisins;
    NOMAD::Voisin mon_voisin;
    mon_voisin.distance = 2.0;
    mon_voisin.origin = 1;
    mes_voisins.insert(mon_voisin);

    mon_voisin.distance = 0.5;
    mon_voisin.origin = 2;
    mes_voisins.insert(mon_voisin);
    /* if ( signature )
                            {
    ...
    NOMAD::Double knn_score;
    NOMAD::Double best_method_score;
    // save _eval_lop in pts
    // ------------------------------------
    const NOMAD::Eval_Point * y;
    std::list<const NOMAD::Eval_Point *> pts;
    ...
    for (i = 0; i < n; ++i) {
        ...
        interpolation_radius[i] = (max[i] - min[i]) / 2.0;
    }
    ...
    for (it2 = pts.begin(); it2 != end2; ++it2)// On parcourt la lop
    {
        x = &NOMAD::Cache::get_modifiable_point(**it2);
        NOMAD::Double knn_score;
        std::set <NOMAD::Voisin> voisins;
        NOMAD::Voisin un_voisin;

        //On parcourt la cache
        ...
        while (cur) {
            //std::cout<< n<< "\n";
            compteur++;
            //std::cout<<compteur<< *cur;
            //sleep(1);
            //std::cout<<"passes tu ici?";
            //std::cout << count<< "\n ";
            //count++;
            //if ( cur->get_eval_status() == NOMAD::EVAL_OK 
            //cur->get_n() == _n             &&
            //cur->get_n() == x2.size()             &&
            //x1.get_signature()->is_compatible (*cur)          &&          
            //)

            /* {
                 if (cur->is_feasible(0)){
       distances_a_y_et_realisabilite.push_back(2);
       distances_a_y_et_realisabilite[distances_a_y_et_realisabilite.size()][0]=(*y-*cur).norm(); 
      distances_a_y_et_realisabilite[distances_a_y_et_realisabilite.size()][1]=0.0;
       }
      else {distances_a_y_et_realisabilite.push_back(2);
      distances_a_y_et_realisabilite[distances_a_y_et_realisabilite.size()][0]=(*y-*cur).norm(); 
      distances_a_y_et_realisabilite[distances_a_y_et_realisabilite.size()][1]=1.0; }
  
             */


            //         std::cout<< "test for "<< classe_voisin;
            //int s;
            //cin >>s;   
            ...
            // on calcule la norme normalisé par les deltas
            NOMAD::Double squared_norm_normalised = 0.0;
            //std::cout<< n<< "\n";
            for (i = 0; i < n; i++) {
                //std::cout<< n<< "\n";

                if (interpolation_radius[i] != 0) {
                    squared_norm_normalised += (((*x)[i]-(*cur)[i]) / interpolation_radius[i]).pow2();
                }
            ...
            un_voisin.distance = squared_norm_normalised.sqrt();
            ...
            voisins.insert(un_voisin);

            //std::cout<< "nb_voisins "<< classe_voisin.size() << "\n";
            //std::cout<< "classe "<< classe_voisin[classe_voisin.size()-1] << "\n";
            //std::cout<<"passe tu ici?";
            //std::cout<<compteur<< *cur<<"avant";
            //std::cin.get();
            cur = _cache->next();
            //if (cur)
            //std::cout<<compteur<< *cur<<"apres";
            //else std::cout<<"plus d'éléments dans la cache";
            //std::cin.get();
            //std::cout<<"oui \n \n";
            //knn_score=(double)(rand()% 1000)/(double)(1000); // On effectue un tirage uniforme entre 0 et 1.     
            // On récupère le plus proche voisin
        ...
        //std::cout<<"le nombre d'elements dans la cache est"<< compteur<< " \n \n";

        knn_score = 0.0;

        std::set<NOMAD::Voisin>::const_iterator it222;

        it222 = voisins.begin();

        knn_score = (*it222).distance;
        //std::cout<<"avant"<<"\n";
        //std::cin.get();
        ...
        //std::cout << " \n";
        best_method_score = knn_score;
        x->set_user_eval_priority(best_method_score);
        add_eval_point(x,
                display_degree,
                snap_to_bounds,
                NOMAD::Double(),
    ...
    //const NOMAD::Eval_Point *            yy;

    //const NOMAD::Signature  *            signature     = NULL;


    //std::set<NOMAD:: Priority_Eval_Point>::const_iterator it3 , end3 = _eval_lop.end();
    // for ( it3 = _eval_lop.begin() ; it3 != end3 ; ++it3 )
    //{
    //  yy=it3->get_point();
    // std::cout << (*yy) << "\n \n" ;
    // std::cout << (*yy).get_user_eval_priority()<< "\n ";
                        ...
                        for (it2 = pts.begin(); it2 != end2; ++it2) {
                            ...
                            if (!cautious || model.is_within_trust_radius(scaled_pt)) {
                                ...
                                if (success) {
                                    ...
                                    //vector<NOMAD::Double> maxs=_cache->get_mean_of_each_output();
                                    bool need_to_recalculate = false;
                                    //cout <<"avant 3\n";
    ...
    if (!modified_list && _model_eval_sort && _eval_lop.size() > 1) {
        switch (_p.get_model_eval_sort()) {
            ...
            case NOMAD::SGTELIB_MODEL:
                ...
                break;
            case NOMAD::NO_MODEL:
                if (_p.get_diversification() == 1)
                    classification_adaptated_one_binary_constraint_ordering_basic_diversification_normalised(display_degree, modified_list);
        }
    }
    ...
    for (it = evaluated_pts.begin(); it != end; ++it) {
        if (*it && (*it)->get_mod_use() >= 0) {
            ...
            if (h.is_defined() && mh.is_defined() && f.is_defined() && mf.is_defined()) {
                ...
                (*it)->get_Yw().display(out, "%12.3g");
                out << " ";
                (*it)->get_cond().display(out, "%12.3g");
                out << " ";
                if (_p.has_constraints()) {
                    eh = h.rel_err(mh) * 100.0;
                    h.display(out, "%14.3g");
                    out << " ";
                    mh.display(out, "%14.3g");
                    out << " ";
                    eh.display(out, "%14.3g");
                    out << " ";
                }
                f.display(out, "%14.3g");
                out << " ";
                mf.display(out, "%14.3g");
                out << " ";
